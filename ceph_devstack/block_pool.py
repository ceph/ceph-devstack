...
class BlockPool:
    ...
    def _has_partition_siblings(name: str) -> bool:
        ...
        if not BlockPool._is_whole_disk(name):
            return False
    ...
    @classmethod
    def _device_mounted(cls, parent: str) -> bool:
        proc = cls._run(["findmnt", "-n", "-o", "TARGET", "--source", parent])
        return proc.returncode == 0 and bool(proc.stdout.strip())

    @classmethod
    def _device_has_blkid_signature(cls, parent: str) -> bool:
        proc = cls._run(["blkid", "-p", "-o", "export", parent])
    ...
    @classmethod
    def _tail_marker_offset(cls, parent: str) -> int:
        size = cls._device_size_bytes(parent)
        if size < _TAIL_MARKER_SIZE:
            raise BlockPoolError(f"block pool parent {parent!r} is too small")
        return size - _TAIL_MARKER_SIZE

    @classmethod
    def _read_tail_marker(cls, parent: str) -> dict | None:
        offset = cls._tail_marker_offset(parent)
        raw = cls._read_device(parent, offset, _TAIL_MARKER_SIZE)
    ...
    @classmethod
    def _write_tail_marker(cls, parent: str, payload: dict) -> None:
        offset = cls._tail_marker_offset(parent)
        ...
        cls._write_device(parent, offset, raw)

    @classmethod
    def _probe_parent_empty(cls, parent: str) -> None:
        size = cls._device_size_bytes(parent)
        offsets = [0]
        if size > 512:
            offsets.append(512)
        ...
        tail = cls._tail_marker_offset(parent)
        if tail not in offsets and tail >= _PROBE_SIZE:
            offsets.append(tail)
        for offset in offsets:
            data = cls._read_device(parent, offset, _PROBE_SIZE)
            if not cls._is_region_empty(data):
                raise BlockPoolError(
    ...
    def _verify_enrolled_parent(self) -> None:
        marker = self._read_tail_marker(self.parent)
        if marker is None:
            raise BlockPoolError(
    ...
    def _enroll_new_parent(self) -> None:
        ...
        if self._device_mounted(self.parent):
            raise BlockPoolError(
                f"block pool parent {self.parent!r} is mounted; refusing to enroll"
            )
        if self._device_has_blkid_signature(self.parent):
            raise BlockPoolError(
        ...
        if self._is_whole_disk(parent_name) and self._has_partition_siblings(
            parent_name
        ):
            raise BlockPoolError(
        ...
        self._probe_parent_empty(self.parent)

        payload = {
            "pool_id": self._state["pool_id"],
            "parent": self.parent,
        }
        self._write_tail_marker(self.parent, payload)
        self._state["enrolled"] = True

    def _validate_region(self, offset: int, size: int, owner: str) -> None:
        parent_size = self._device_size_bytes(self.parent)
        ...
        if offset >= self._tail_marker_offset(self.parent):
            raise BlockPoolError(
                f"block pool region for {owner!r} would overlap the enrollment "
                f"marker on {self.parent!r}"
            )
        probe = min(_PROBE_SIZE, size)
        data = self._read_device(self.parent, offset, probe)
        if not self._is_region_empty(data):
            raise BlockPoolError(

...
class TestLoopControlDeviceWriteable:
    ...
    async def test_loop_control_writeable_check_false_local(self):
        ...
        def side_effect_run(args, **kwargs):
            ...
            raise AssertionError(f"unexpected host.run: {args}")

        with (
            patch.object(self.req.host, "arun", return_value=mock_check_proc),
            patch.object(self.req.host, "run", side_effect=side_effect_run),

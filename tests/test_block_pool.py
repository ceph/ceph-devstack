...
class TestBlockPool:
    ...
    def test_enroll_writes_marker_and_state(self, block_pool, mock_fresh_enrollment):
        ...
        marker = BlockPool._read_tail_marker(block_pool.parent)
    ...
    def test_new_slice_rejects_existing_data(self, enrolled_pool):
        with (
            patch.object(enrolled_pool, "_verify_enrolled_parent"),
            patch(
                "ceph_devstack.block_pool.BlockPool._read_device",
    ...
    def test_enroll_rejects_whole_disk_with_partitions(
        self, tmp_path, file_backed_device_size
    ):
        ...
        with (
            patch(
                "ceph_devstack.block_pool.BlockPool._device_mounted", return_value=False
            ),
            patch(
            ...
            patch(
                "ceph_devstack.block_pool.BlockPool._read_tail_marker",
                return_value=None,
            ),
            patch(
                "ceph_devstack.block_pool.BlockPool._has_partition_siblings",
                return_value=True,
            ),
    ...
    def test_reclaims_marker_after_state_loss(self, disallow_enroll_pool):
        ...
        BlockPool._write_tail_marker(disallow_enroll_pool.parent, payload)
        with (
            patch(
                "ceph_devstack.block_pool.BlockPool._device_mounted", return_value=False
            ),
...
class TestBlockPoolEnrollmentGuards:
    def test_enroll_rejects_mounted_parent(self, block_pool, mock_fresh_enrollment):
        with (
            patch(
                "ceph_devstack.block_pool.BlockPool._device_mounted", return_value=True
            ),
...
class TestPartitionSiblings:
    def test_whole_disk_with_nvme_partitions(self):
        ...
        with patch("ceph_devstack.block_pool.Path", side_effect=path_for):
            assert BlockPool._has_partition_siblings("nvme0n1") is True

    def test_partition_name_has_no_siblings(self):
        assert BlockPool._has_partition_siblings("nvme0n1p1") is False
...
class TestTailMarker:
    def test_roundtrip(self, tmp_path, file_backed_device_size):
        parent = tmp_path / "disk"
        parent.write_bytes(b"\x00" * (1024 * 1024))
        payload = {"pool_id": "abc", "parent": str(parent)}
        BlockPool._write_tail_marker(str(parent), payload)
        assert BlockPool._read_tail_marker(str(parent)) == payload

    def test_corrupt_marker_returns_none(self, tmp_path, file_backed_device_size):
        parent = tmp_path / "disk"
        parent.write_bytes(b"\x00" * (1024 * 1024))
        ...
        assert BlockPool._read_tail_marker(str(parent)) is None
